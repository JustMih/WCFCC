...
const determineWorkflowPath = (complaintType, unitName) => {
  const isDirectorate =
    unitName && unitName.toLowerCase().includes("directorate");
...
const getAvailableUsers = async (role, unitName = null) => {
  ...
  users.forEach((user) => {
    console.log(
      `- ${user.full_name} (${user.email}) - Role: "${user.role}" - Unit: "${user.unit_section}"`
    );
  });
  ...
  if (users.length === 0) {
    ...
    for (const fallbackRole of fallbackRoleList) {
      ...
      console.log(
        `\nTrying fallback role: "${fallbackRole}" with where clause:`,
        JSON.stringify(fallbackWhereClause, null, 2)
      );
      ...
      console.log(
        `Found ${users.length} users with fallback role "${fallbackRole}":`
      );
      users.forEach((user) => {
        console.log(
          `- ${user.full_name} (${user.email}) - Role: "${user.role}" - Unit: "${user.unit_section}"`
 ...
 */
const assignToNextInWorkflow = async (
  ticketId,
  currentUserId,
  workflowPath,
  unitName,
  ...
  if (availableUsers.length === 0) {
    throw new Error(
      `No users available for role: ${nextRole} or any fallback roles`
    );
  }
 ...
 */
const handleComplaintRating = async (
  ticketId,
  complaintType,
  unitName,
  coordinatorId,
 ...
 */
const handleComplaintToInquiry = async (
  ticketId,
  coordinatorId,
  justification
) => {
...
const getWorkflowStatus = async (ticketId) => {
  ...
  const workflowPath = determineWorkflowPath(
  ...
  return {
    ...
    nextRole:
      currentStepIndex < totalSteps - 1
        ? workflow.roles[currentStepIndex + 1]
        : null,
...
const coordinatorActions = {
  ...
  assignToNext: async (ticketId, coordinatorId, justification = "") => {
    ...
    if (availableUsers.length === 0) {
      throw new Error(
        `No Head of Unit available for unit: ${ticket.responsible_unit_name}`
      );
    }
...
const assignToNextRole = async (
  ...
  const currentRole = ticket.assigned_to_role;
  const nextRole = getNextRoleByCurrentRole(
    currentRole,
    ticket.complaint_type,
    ticket.responsible_unit_name
  );
